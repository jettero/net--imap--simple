...
Returns numbers of messages that are smaller than C<< <x> >> octets.  This
function will try to force your argument to be a number before passing it to the
IMAP server.

=item search_larger

Returns numbers of messages that are larger than C<< <x> >> octets.  This
function will try to force your argument to be a number before passing it to the
IMAP server.
...
=item search_before
=item search_since
=item search_sentbefore
=item search_sentsince
